...
class AdminOwnerRepliesController extends MyController {
	...
	var $uses = array('menu','user','owner_reply','predefined_reply','review','criteria');
	...
	function moderation()
    {
		...
		$replies = $this->OwnerReply->findAll(array(
            'fields'=>array(
                'CASE WHEN CHAR_LENGTH(User.' . UserModel::_USER_REALNAME . ') THEN User.' . UserModel::_USER_REALNAME . ' ELSE OwnerReply.name END AS `User.name`',
