...
{
    var $uses = array('user','claim','predefined_reply','menu','field','criteria');
    ...
    function moderation()
    {
        ...
        $claims = $this->Claim->findAll(array(
            'fields'=>array(
                'Claim.*',
                'User.' . UserModel::_USER_REALNAME . ' AS `Claim.name`',
                'User.' . UserModel::_USER_EMAIL . ' AS `Claim.email`'
            ),
            'conditions'=>$conditions,
            'joins'=>array(
                'LEFT JOIN ' . UserModel::_USER_TABLE . ' AS User ON User.' . UserModel::_USER_ID . ' = Claim.user_id'
            ),
            'offset'=>$this->offset,
            'limit'=>$this->limit,
       ...
       if(!empty($claims))
       {
            ...
            $listings = $this->Listing->findAll(array(
                'conditions'=>array(
                    'Listing.' . EverywhereComContentModel::_LISTING_ID . ' IN (' . implode(',',$listing_ids) . ')'
                )
            ),array('afterFind'));
