...
class AdminMediaUploadController extends MyController
{
	...
	function _save()
	{
		...
		$listing = $this->Listing->findRow(array('conditions'=>array('Listing.id = ' . (int) $listing_id)), array('afterFind'));
	...
	function _uploadUrl()
	{
		...
		if($uploaded_media_info)
		{
			...
			// Don't run callbacks to update the media counts
			// because we run them only when the encoding job is done
			$this->Media->store($this->data,false,array('plgBeforeBeforeSave','beforeSave','plgBeforeSave','plgAfterSave'));
	...
	private function _processAttachment($media)
	{
		...
		$result = array(
			...
			'thumb_url'=>ThemingComponent::getImageUrl($file)
		);
