...
class MediaController extends MyController
{
    ...
    var $helpers = array('libraries','html','text','assets','paginator','jreviews','widgets','form','routes','time','community','media');
	...
	function _setPhotoFunction()
	{
		$response = array('success'=>false,'str'=>array());

		$sessionUser = UserAccountComponent::getUser();

        if($this->_user->id === 0 && empty($sessionUser))
        {
        	return cmsFramework::jsonResponse($response);
        }

		$mediaId = Sanitize::getInt($this->params, 'id');

		$function = Sanitize::getString($this->params, 'function');

		if(!$mediaId || !in_array($function, array('gallery', 'cover', 'logo'))) return cmsFramework::jsonResponse($response);

		$photo = $this->Media->findRow(array('conditions' => array(
				'Media.media_id = ' . (int) $mediaId),
				'Media.media_type = "photo"'
			),
			$callbacks = array()
		);

		if(!$photo) return cmsFramework::jsonResponse($response);

		if(!cmsFramework::isAdmin())
		{
			# Stop form data tampering
			$formToken = cmsFramework::formIntegrityToken($photo['Media'],array_keys($this->formTokenKeysEdit),false);
			...
			if ($this->_user->id > 0)
			{
				$listingOwner = $this->Listing->getListingOwner($photo['Media']['listing_id']);

				$canManageMedia = $this->Access->canManageMedia($photo['Media']['media_type'],$photo['User']['user_id'],$listingOwner['user_id']);

				if(!$canManageMedia) return cmsFramework::jsonResponse($response);
			}
		}

		$res = $this->Media->setPhotoFunction($photo, $function);
	...
	function mediaList($params = array(), $conditions = array())
	{
        ...
        *******************************************************************/
		...
		if($this->action == 'myMedia')
		{
			$page['show_title'] = $this->page_params['show_title'];

			$page['title'] =
	            $page['title_seo'] = $this->page_params['title'];
		}
	...
	function listing()
	{
		...
		$this->Theming->setSuffix(array('listing_id' => $listing_id));
	...
	function myMedia()
	{
		...
		if($user_id_param > 0)
		{
			...
			$user_name = $this->User->findOne(
				array(
					'fields'=>array('User.' . ($name_choice == 'username' ? UserModel::_USER_ALIAS : UserModel::_USER_REALNAME) . ' AS `User.name`'),
	...
	function download()
	{
		$no_access = '<script>window.parent.jQuery.jrAlert(window.parent.jreviews.__t("ACCESS_DENIED"));</script>';

		$file_not_found = '<script>window.parent.jQuery.jrAlert(window.parent.jreviews.__t("MEDIA_DOWNLOAD_NOT_FOUND"));</script>';

		$invalid_token = '<script>window.parent.jQuery.jrAlert(window.parent.jreviews.__t("INVALID_TOKEN"));</script>';
		...
		# Stop form data tampering
		$formToken = cmsFramework::getCustomToken((int)$media['Media']['media_id'],$media['Media']['media_type'],$media['Media']['filename'], $media['Media']['created']);
	...
	function reorder()
	{
		...
		# Stop form data tampering
		$formToken = cmsFramework::getCustomToken($listing_id . $extension);
	...
	function videoGallery()
	{
		...
		$m = Sanitize::getString($this->params,S2_QVAR_MEDIA_CODE);
	...
	function setMainMedia()
	{
		...
		if(!cmsFramework::isAdmin())
		{
			...
			if ($this->_user->id > 0)
			{
				...
				$canManageMedia =  $this->Access->canManageMedia($media['Media']['media_type'],$media['User']['user_id'],$listingOwner['user_id']);
