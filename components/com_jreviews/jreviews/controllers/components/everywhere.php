...
class EverywhereComponent extends S2Component {
    ...
    var $validObserverModels = array('Media','Review','ReviewReport','Inquiry','Discussion');
    ...
    function startup(&$controller) {
        ...
        {
            /**
             * Not very elegant, but for the discussions controller we need to override the Everywhere model for the 'review' action
             * so it uses the Review model instead of the Discussion model
             */
            if($controller->name == 'discussions' && $controller->action == 'review')
            {
                $this->everywhereModel = $controller->Review;
            }
            else {
	...
	function runEverywhereModels($models, $extensions)
	{
		...
		foreach($models AS $extension=>$EveryWhereModel)
        {
			...
			if(!file_exists(S2Paths::get('jreviews', 'S2_MODELS_CMS') . $this->name . DS . $EveryWhereModel . '.php')
                &&
                !file_exists(PATH_APP_OVERRIDES . DS . 'cms_compat' . DS . _CMS_NAME . DS . 'models' . DS . 'everywhere' . DS . $EveryWhereModel . '.php'))
            {
		...
		foreach($extensions AS $extension=>$listing_ids)
		{
			if(isset($models[$extension]))
			{
                ...
                $listings[$extension] = $this->{inflector::camelize($models[$extension])}->findAll(array('conditions' => array('Listing.' . $realKey . ' IN ('. implode(',',$listing_ids) . ')')));
			}
		}
        ...
        switch($this->everywhereModel->name)
        {
            ...
            case 'Discussion':
                ...
                if(isset($this->c->EverywhereAfterFind) &&  $this->c->EverywhereAfterFind === true)
                {
					...
					if($this->c->action != '_save' && $this->everywhereModel->name == 'Review')
					{
                    ...
                    foreach($results AS $key=>$result)
                    {
                        if(isset($listings[$result['Review']['extension']])
                            ...
                            ||
                            (
                                // In review moderation we want to include the orphaned reviews so they can be deleted
                               !isset($listings[$result['Review']['extension']][$result['Review']['listing_id']])
                                && $this->controller_action == 'moderation'
                            )
                        )
                        {
                            // Second condition above excludes reviews when the listing is not found for the review (i.e. the category for the listing was removed)
                            ...
                            if(isset($listings[$result['Review']['extension']][$result['Review']['listing_id']]))
                            {
...
//        appLogMessage('New: '. $model->data['new'],'plgAfterSave');
    ...
    function loadListingModel(&$controller, $extension = null)
    {
	   ...
	   if(!$extension && isset($this->everywhereModel) && is_object($this->everywhereModel)) {
       ...
       $extension == ''
		...
		if($extension)
        {
            ...
            if(class_exists($class_name))
            {
                ...
                $controller->Listing->passedArgs = $controller->passedArgs;
