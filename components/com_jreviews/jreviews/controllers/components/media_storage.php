...
//			,'facebook'=>'facebook'
	...
	function delete($data)
	{
		...
		foreach($data AS $media_id=>$media)
		{
			...
			if(isset($media_info['attachment'])) {
				$attachment_url = $media['Media']['media_path'] . '.' . $media['Media']['file_extension'];
				if($this->_STORAGE_CLASSES[$storage_service]->delete($attachment_url, $media)) {
					$errors[$media_id][] = $attachment_url;
				}
			}
		}
