...
class NotificationsComponent extends S2Component {
        ...
        switch($this->notifyModel->name)
        {
            ...
            case 'Media':
                ...
                if(($this->c->_user->id == $media['User']['user_id'] || Sanitize::getBool($this->c->params,'remote') == 1) && $media['User']['user_id'] > 0 && $media['User']['email'] != '')
                {
                    ...
                    $message = $this->c->partialRender('email_templates','user_media_encoding_notification','email');
            ...
            case 'Listing':
                ...
                if ($this->c->Config->notify_content)
                {
                    ...
                    $message = $this->c->partialRender('email_templates','admin_listing_notification','email');
                ...
                if ($this->c->Config->notify_user_listing)
                {
                    ...
                    if($this->c->_user->id == $listing['User']['user_id'])
                    {
                       ...
                       // Process configuration emails
                        ...
                        $message = $this->c->partialRender('email_templates','user_listing_notification','email');
            ...
            case 'Review':
                ...
                if ($this->c->Config->notify_review)
                {
                    ...
                    $message = $this->c->partialRender('email_templates','admin_review_notification','email');
                ...
                if(
                    ...
                    if($this->c->_user->id == $review['User']['user_id'])
                    {
                        ...
                        $message = $this->c->partialRender('email_templates','user_review_notification','email');
                ...
                )
                {
                    ...
                    $message = $this->c->partialRender('email_templates','owner_review_notification','email');
            ...
            case 'OwnerReply':
                ...
                if ($this->c->Config->notify_owner_reply
                        || $this->c->Config->notify_reviewer_owner_reply
                    ) {
                    ...
                    $review['Listing'] = & $listing['Listing'];
                ...
                if ($this->c->Config->notify_owner_reply)
                {
                    ...
                    $message = $this->c->partialRender('email_templates','admin_owner_reply_notification','email');
                ...
                # Owner reply email to reviewer

                if ($this->c->Config->notify_reviewer_owner_reply)
                {
                    $this->_clearAddresses($mail);

                    // Process configuration emails
                    $this->_AddBCC($mail, 'notify_reviewer_owner_reply_emails');

                    trim($review['User']['email']) != '' and $mail->AddAddress(trim($review['User']['email']));

                    $subject = sprintf(JreviewsLocale::getPHP('NOTIFY_NEW_OWNER_REPLY'),$review['Listing']['title']);

                    $message = $this->c->partialRender('email_templates','reviewer_owner_reply_notification','email');
                    ...
                    {
                       appLogMessage(array(
                               "Owner reply to reviewer message was not sent.",
                               "Mailer error: " . $mail->ErrorInfo),
                               'notifications'
                           );
                    }
            ...
            case 'Report':
                ...
                if ( $this->c->Config->notify_report ) {
                    ...
                    # Process configuration emails
                    if($this->c->Config->notify_report_emails == '')
                    {
                        $mail->AddAddress($configMailFrom);
                    }
                    else
                    {
                        $recipient = explode("\n",$this->c->Config->notify_report_emails);
                    ...
                    $mediaId = Sanitize::getInt($model->data['Report'], 'media_id');

                    $reviewId = Sanitize::getInt($model->data['Report'], 'review_id');

                    $postId = Sanitize::getInt($model->data['Report'], 'post_id');

                    $listingId = Sanitize::getInt($model->data['Report'], 'listing_id');

                    $extension = Sanitize::getString($model->data['Report'], 'extension');

                    if($postId)
                    {
                        $post = $this->c->Discussion->findRow(array(
                            'conditions'=>array(
                                'Discussion.type = "review"',
                                'Discussion.discussion_id = ' . $postId
                                ), $callbacks = array())
                        );
                    ...
                    if($reviewId)
                    {
                        $review = $this->c->Review->findRow(array('conditions'=>array('Review.id = ' . $reviewId)), $callbacks = array());
                    }

                    if($mediaId)
                    {
                        $media = $this->c->Media->findRow(array('conditions'=>array('Media.media_id = '. $mediaId)), $callbacks = array());

                    }

                    # Load Everywhere extension model

                    if($extension == '' && $reviewId)
                    {
                        $extension = $review['Review']['extension'];
                    }
                    elseif($extension == '' && $mediaId) {
                    ...
                    $listing = $EverywhereListingModel->findRow(array('conditions'=>array("Listing.$EverywhereListingModel->realKey = " . $listingId)), $callbacks = array());

                    $subject = sprintf(JreviewsLocale::getPHP('NOTIFY_NEW_REPORT'), self::reportType($reviewId, $mediaId, $postId));
                    ...
                    $message = $this->c->partialRender('email_templates','admin_report_notification','email');
            ...
            case 'Discussion':
                ...
                // Perform common actions for all review notifications
                if(Sanitize::getBool($this->c->Config,'notify_review_post') // admin
                    ||
                    Sanitize::getBool($this->c->Config,'notify_user_comment') // to submitter
                    ||
                ...
                // Admin email
                if(Sanitize::getBool($this->c->Config,'notify_review_post'))
                {
                    $this->_clearAddresses($mail);
                    ...
                    $message = $this->c->partialRender('email_templates','admin_review_discussion_post','email');
                ...
                if(
                    ...
                    if($this->c->_user->id == $post['User']['user_id'])
                    {
                        ...
                        $message = $this->c->partialRender('email_templates','user_comment_notification','email');
                ...
                if(
                    ...
                    $message = $this->c->partialRender('email_templates','reviewer_comment_notification','email');
                ...
                if(
                    ...
                    $message = $this->c->partialRender('email_templates','owner_comment_notification','email');
            ...
            case 'Claim':
                ...
                if ($this->c->Config->notify_claim ) {
                    ...
                    $listing = $this->c->Listing->findRow(
                        array(
                            'conditions'=>array('Listing.'.EverywhereComContentModel::_LISTING_ID.' = ' . (int) $model->data['Claim']['listing_id'])
                        ),
                        $callbacks
                    );
                    ...
                    $message = $this->c->partialRender('email_templates','admin_listing_claim','email');
    ...
    public static function reportType($reviewId, $mediaId, $postId)
    {
        if($mediaId) {
    ...
    {
        // Jan 16, 2017 - No longer loading the listing from the viewVars array because it can have
        // incorrect information in the case where the listing alias is generated from custom fields
        ...
        $listing = $this->c->Listing->findRow(array('conditions'=>array('Listing.'.EverywhereComContentModel::_LISTING_ID.' = '. $listing_id)),array('afterFind' /* Only need menu id */));
