...
//ftp://[user[:password]@]hostname[:port]/[path]/[filename][?passive=yes]
	   ...
	   // Preparing XML request
		...
		{
			$notify_url = cmsFramework::getAjaxUriAbs() . '&url=media_upload/checkJobStatus&remote=1&media_id='.$media_id.'&token='.$notify_token;

			$req->addChild('notify', htmlspecialchars($notify_url));
		}
