...
// Troubleshooting
    ...
    var $db_index_hints_disable = false;
...
// Community
    ...
    var $social_sharing_disable_secure_urls = 1;
    // Jomsocial integration
    var $jomsocial_tnmode =  'crop';
    var $jomsocial_tnsize =  '65x65';
...
//item list tab
	var $list_predefined_layout = array(
		1=>array('layout'=>'blogview','suffix'=>'','icon'=>'jrIconList'),
		2=>array('layout'=>'','suffix'=>'','icon'=>'jrIconTable'),
		3=>array('layout'=>'','suffix'=>'','icon'=>'jrIconThumbs')
		);
	var $list_show_addnew	= 1;
	var $list_show_page_catimage = 1;
	var $list_show_catimages = 1;
    var $list_addnew_menuid = 1; // Keep current itemid in url
	var $list_show_searchbox = 1;
	var $list_show_orderselect = 1;
	var $bayesian_exclude_unrated = 1;
	var $user_review_bayesian = 0;
	var $user_review_bayesian_tuning = 0.25;
	var $editor_review_bayesian = 0;
	var $editor_review_bayesian_tuning = 0.25;
	var $list_order_default	= "alpha";
	var $list_order_options = array('featured','alpha','rdate','updated','rhits','rating','editor_rating','reviews');
	var $list_order_criteria = 'user';
    ...
    // listings tab
    var $media_photo_function_logo = 0;
    var $media_photo_function_cover = 0;
    var $listing_detail_header_empty_cover = 0;
    var $listing_detail_header_empty_cover_color = '#5191E8';
    var $listing_detail_header_empty_logo = 0;
    var $listing_detail_header_empty_logo_color = '#CCC';
    var $listing_detail_header_avatar = '0';
    ...
    var $review_order_options = array('rdate','rating','rrating','helpful','discussed');
    var $review_comment_wysiwyg = 0; // Trix editor
        //=> ratings
    var $rating_scale = "5";
    var $rating_increment = 1;
    var $rating_selector = "stars"; // stars
    var $rating_selector_mobile = "slider"; // stars
    var $editor_rating_style = 1;
    var $editor_rating_color = 'orange';
    var $user_rating_style = 1;
    var $user_rating_color = 'green';
    var $rating_graph = 1;
	...
	//standard fields tab
	var $content_title_duplicates = 'yes';
	...
	var $listing_wysiwyg = 0; // Trix editor
    ...
    var $discussion_wysiwyg = 0; // Trix editor
    ...
    var $notify_reviewer_owner_reply = 0;
    var $notify_reviewer_owner_reply_emails;
    ...
    var $media_upload_protocol = 'post';
	var $media_url_catchall = 1;
	var $media_general_default_image_path;
	var $media_general_default_video_path;
	...
	var $media_attachment_show_count = 0;

	var $media_audio_show_count = 0;

	// Display

	var $media_display_more_by = 0;

	var $media_report_abuse = 1;
	...
	var $media_article_sync = 'none';

	var $media_article_hide = 'all';
	...
	var $media_photo_resize = '1500x1500';

	var $media_photo_resize_quality = 90;

	// Videos

	var $media_youtube_key;
	...
	var $lang_listing_edit = "LANG_LISTING_EDIT";

	var $lang_listing_featured = "LANG_LISTING_FEATURED";

	var $lang_listing_send_inquiry = "LANG_LISTING_SEND_INQUIRY";

	// review general

	var $lang_review_write_detail = "LANG_REVIEW_WRITE_DETAIL";

	var $lang_review_add_detail = "LANG_REVIEW_ADD_DETAIL";

	var $lang_review_comment_add_detail = "LANG_REVIEW_ADD_COMMENT_DETAIL";

	var $lang_review_user_reviews_not_found = "LANG_REVIEW_USER_REVIEWS_NOT_FOUND";

	var $lang_review_user_comments_not_found = "LANG_REVIEW_USER_COMMENTS_NOT_FOUND";

	// listing form

	var $lang_listing_create = "LANG_LISTING_CREATE";

	var $lang_listing_form_title = "LANG_LISTING_FORM_TITLE";

	var $lang_listing_form_title_search = "LANG_LISTING_FORM_TITLE_SEARCH";

	var $lang_listing_form_title_alias = "LANG_LISTING_FORM_TITLE_ALIAS";

	var $lang_listing_form_upload_media_info = "LANG_LISTING_FORM_UPLOAD_MEDIA_INFO";

	var $lang_listing_form_summary = "LANG_LISTING_FORM_SUMMARY";

	var $lang_listing_form_description = "LANG_LISTING_FORM_DESCRIPTION";

	// media form

	var $lang_listing_media_upload_header = "LANG_LISTING_MEDIA_UPLOAD_HEADER";

	var $lang_review_media_upload_header = "LANG_REVIEW_MEDIA_UPLOAD_HEADER";

	var $lang_media_photo_function_gallery = "LANG_MEDIA_PHOTO_FUNCTION_GALLERY";

	var $lang_media_photo_function_logo = "LANG_MEDIA_PHOTO_FUNCTION_LOGO";

	var $lang_media_photo_function_cover = "LANG_MEDIA_PHOTO_FUNCTION_COVER";


	function startup(&$controller = null)
	{
		if(Sanitize::getString($this,'controller_name') == 'admin/install') return;

		$cms_version = cmsFramework::getVersion();

		$this->media_general_default_image_path = PATH_APP_REL . '/views/themes/default/theme_images/nophoto.png';

		$this->media_general_default_video_path = PATH_APP_REL . '/views/themes/default/theme_images/novideo.png';

		// Change
		if(_CMS_NAME == 'joomla' && $cms_version >= 3) {
        ...
        elseif(_CMS_NAME == 'wordpress') {
        	...
        	// $this->media_article_sync = 'none';

        	$this->user_registration_guest = 0;
        }
		...
		if($Config = Configure::read('JreviewsSystem.Config'))
		{
			...
			if(Sanitize::getString($Config,'cms_secret') == '')
			{
				$cms_secret = cmsFramework::getConfig('secret');

				$Config->cms_secret = $cms_secret;

				$this->store(array('cms_secret'=>$cms_secret));
			}
		...
		if(!defined('S2_NO_SSL'))
		{
			$User = cmsFramework::getUser();

			if(!empty($_SERVER['HTTPS']) && (($this->url_no_ssl == 'guests' && $User->id == 0) || $this->url_no_ssl == 'all'))
			{
				define('S2_NO_SSL', 1);
			}
			else {
	...
	function load()
    {
		...
		$query = "SELECT id, value FROM #__jreviews_config";

		$rows = $Model->query($query, 'loadObjectList');
		...
		if($rows)
		{
			foreach ($rows as $row)
			{
                ...
                if((substr($row->value,0,1)=='[' && substr($row->value,$length,1)==']')
                	||
                	substr($row->value,0,1)=='{' && substr($row->value,$length,1)=='}'
                	)
                {
		...
		# Migrate old 'list_display_type' setting to the predefined layout settings.

		if(isset($Config->list_display_type) && !isset($Config->list_predefined_layout))
		{
	        switch($Config->list_display_type) {

	            case 0:
	                $default_layout = 'tableview';
	                break;
	            case 1:
	                $default_layout = 'blogview';
	                break;
	            case 2:
	                $default_layout = 'thumbview';
	                break;
	            case 3:
			...
			$Config->list_predefined_layout = array(
				1=>array('layout'=>$default_layout,'suffix'=>'','icon'=>'jrIconList'),
				2=>array('layout'=>'','suffix'=>'','icon'=>'jrIconTable'),
				3=>array('layout'=>'','suffix'=>'','icon'=>'jrIconThumbs')
				);
		}

        $Config->rss_title = Sanitize::getString($Config,'rss_title') != '' ? $Config->rss_title : $Model->makeSafe(cmsFramework::getConfig('sitename'));

        $Config->rss_description = Sanitize::getString($Config,'rss_description') != '' ? $Config->rss_description : $Model->makeSafe(cmsFramework::getConfig('MetaDesc'));

		# Get current version number

		$version = cmsFramework::getAppVersion('jreviews');
	...
	function store($arr = null, $arrayToJson = true)
	{
		...
		while (list($prop, $val) = each($arr))
		{
			if($prop != 'c')
			{
				...
				$values[] = '(' . $Model->Quote($prop) . ',' . $Model->Quote($val) . ')';
            }
		}

		if(!empty($values))
		{
        	$fmtquery = "INSERT INTO #__jreviews_config ( id, value ) VALUES %s ON DUPLICATE KEY UPDATE value = VALUES(value)";

			$query = sprintf( $fmtquery, implode( ",", $values ) );
