...
class ComContentController extends MyController {
    ...
    function com_content_view($passedArgs)
    {
        ...
        $metadesc and $content_row->metadesc = htmlspecialchars($metadesc,ENT_COMPAT,'UTF-8');

        $metakey and $content_row->metakey = htmlspecialchars($metakey,ENT_COMPAT,'UTF-8');
        ...
        if(_CMS_NAME == 'joomla')
        {
            ...
            if(is_object($content_row) && isset($content_row->images))
            {
        ...
        elseif(_CMS_NAME == 'wordpress') {
            ...
            $this->Media->updatePostFeaturedImage($listing);
        }
