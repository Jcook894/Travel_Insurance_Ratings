...
class MediaUploadController extends MyController
{
    ...
    function create()
    {
        ...
        if(Sanitize::getBool($this->Config,'media_general_tos') && $tos_id = Sanitize::getInt($this->Config,'media_general_tos_articleid'))
		{
            $tos_article = $this->Article->findRow(array(
                'conditions'=>array
                (
                    'Article.' . ArticleModel::_ARTICLE_ID . ' = ' . $tos_id
                )
            ));
        }
	...
	function _save()
	{
		...
		$listing = $this->Listing->findRow(array('conditions'=>array('Listing.' . constant(get_class($this->Listing) . '::_LISTING_ID') . ' = ' . (int) $listing_id)), array('afterFind'));
		...
		if($uploaded_media_info)
		{
			...
			if($processResults && Sanitize::getBool($processResults,'success'))
			{
				...
				// Update core article images column using the main media

				if($this->data['Media']['extension'] == 'com_content' && Sanitize::getInt($this->data['Media'],'main_media') == 1)
				{
					$media = array('Media'=>array(
						'listing_id'=>$this->data['Media']['listing_id'],
						'title'=>$this->data['Media']['title'],
						'extension'=>'com_content',
						'media_info'=>array('image'=>array('url'=>$processResults['thumb_url']))
					));

					$this->Media->updateListingCoreImage($media);
				}
				...
				$response = array(
					...
					'listing_id'=>$listing_id,
					'review_id'=>$review_id,
					'main_media'=>Sanitize::getInt($this->data['Media'],'main_media'),
					'media_type'=>$media_type,
	...
	function _uploadUrl()
	{
		...
		if(in_array($media_type,array('photo','attachment'))) {
			...
			if(!$local_file = $this->_grabRemoteFile(array('filename'=>$pathinfo['filename'],'file_extension'=>$fileext,'media_type'=>$media_type), $upload_url, $this->tmpFolder, true /*skipUpload*/)) {
			...
			$upload_path = $local_file['path'];

			$upload_url = $local_file['url'];
		...
		$uploaded_media_info = array(
				...
				'orig_filename'=>$pathinfo['filename'] . '.' . $fileext,
				'del_original'=>0
			);
	...
	private function _processAttachment($media)
	{
		...
		$filetype_img_url = ThemingComponent::getImageUrl('filetype/128/'.$media['file_extension'].'.png');
	...
	private function _processPhoto($media)
	{
		...
		$isJpg = in_array($ext, array('jpg', 'jpeg'));
		...
		try {
			...
			if ($isJpg
				...
				&& isset($imageinfo['APP1'])
				&& substr($imageinfo['APP1'], 0, 4) == 'Exif') {
		...
		if($isJpg && function_exists('imagerotate') && $degrees !== 0)
		{
		...
		if($job_id)
		{
			...
			switch($encoding['state'])
			{
				case 'finished':
					...
					$media = $this->Media->query("SELECT approved, filename FROM #__jreviews_media WHERE media_id = " . $media_id, 'loadAssoc');

					$data['Media']['media_info'] = json_encode($media_info);

					$data['Media']['published'] = 1;

					$data['Media']['approved'] = $media['approved'];
					...
					// Delete all files starting with this name from the tmp folder
	...
	function _embedVideo()
	{
		...
		if($this->Config->media_store_video_embed == 's3' && $this->Config->media_store_amazons3_key == '') {
	...
	{
		// Remove from fields because otherwise it's treated as integer when saving and converted to zero.
		// We already have the IP address anyway

		unset($this->Media->fields['Media.ipaddress']);
		...
		elseif(isset($this->data['media']) && is_array($this->data['media'])) {
		...
		$secretApiKey = md5(cmsFramework::getConfig('secret'));

		$passedApiKey = Sanitize::getString($data,'api_key');

		$apiCall = $passedApiKey != '' ? true : false;
		...
		if(!$this->ajaxRequest && strcmp($secretApiKey, $passedApiKey) != 0)
		{
			return $this->response(array('success'=>false,'url'=>''), $apiCall);
		}
		...
		/**
		 * If we don't have a URL for the uploaded media, we need to send it back to moderation
		 */

		if(!isset($media['Media']['media_info']['image']['url']) || $media['Media']['media_info']['image']['url'] == '') {
		...
		if(isset($media_info['thumbnail'])
			&& isset($media_info['thumbnail'][$SizeMode])
			&& isset($media_info['thumbnail'][$SizeMode]['url']))
		{
			// If file stored locally, make sure it has not been deleted, otherwise re-create the thumbail

			if(strstr($media_info['thumbnail'][$SizeMode]['url'], WWW_ROOT))
			{
				$tn_path = str_replace(DS,_DS,str_replace(WWW_ROOT, PATH_ROOT,$media_info['thumbnail'][$SizeMode]['url']));

				if(file_exists($tn_path))
				{
					return $this->response(array('success'=>true,'url'=>$media_info['thumbnail'][$SizeMode]['url']), $apiCall);
				}
			}
			else {
		...
		if(!strstr($img_url,WWW_ROOT) && substr($img_url, 0, 4) == 'http') {

			$local_file = $this->_grabRemoteFile($media['Media'], $img_url, $this->tmpFolder, true /*skipUpload*/);

			$img_path = $local_file['path'];
		}
		else {
			...
			if(substr($img_url, 0, 4) != 'http') {
				...
				// Remove relative
				$img_url = WWW_ROOT . ltrim(str_replace(WWW_ROOT_REL, '', $img_url), _DS);
			}
		...
		/**
		 * If we cant'locate the PATH for the uploaded media, we need to send it back to moderation
		 */
		...
		return $this->response($out, $apiCall);
	}
	...
	public function _grabRemoteFile($media_info, $remote, $tmp_dir /*path*/, $skipUpload = false)
	{
		...
		if($curl_info['http_code'] == 200)
		{
			...
			$local_file = array(
				'path'=>$target,
				'filesize'=>$curl_info['download_content_length'],
				'width'=>$size[0],
				'height'=>$size[1],
				'url'=>pathToUrl($target)
			);
	...
	function response($response, $apiCall)
	{
		if ($apiCall)
		{
