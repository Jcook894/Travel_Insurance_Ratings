...
class MediaModel extends MyModel {
	...
	function __construct()
	{
	    ...
	    $this->fields = array(
					        ...
					        'Media.media_function AS `Media.media_function`',
		...
		'Media.ipaddress'=>'INET_NTOA(Media.ipaddress) AS `Media.ipaddress`',
							...
							'Media.user_id AS `Media.user_id`',
							'Media.user_id AS `User.user_id`',
							'User.' . UserModel::_USER_REALNAME . ' AS `User.name`',
							'User.' . UserModel::_USER_ALIAS . ' AS `User.username`',
							sprintf('IF(User.%1$s <> "", User.%1$s, Media.email) AS `User.email`',UserModel::_USER_EMAIL)
	    );

	    $this->joins = array(
	        'User'=>'LEFT JOIN #__users AS User ON Media.user_id = User.' . UserModel::_USER_ID
	    );
	...
	function del($media_ids)
	{
		...
		foreach($this->data_after_delete AS $media)
		{
			$media['media_id'] = $media['Media']['media_id'];

			$this->data = $media;

			$this->plgBeforeDelete($media);
		}
	...
	function afterDeleteMedia($media_id = null)
	{
		...
		foreach($this->data_after_delete AS $data)
		{
			...
			$this->removeListingCoreImage($data);
		}
	}

	function setPhotoFunction($photo, $function)
	{
		$mediaId = $photo['Media']['media_id'];

		// If no function selected, then we clear the value for the selected photo
		...
		if($function == 'gallery')
		{
			...
			return $this->store($data, $updateNulls = true);
		}

		$listingId = $photo['Media']['listing_id'];

			$query = "
				UPDATE #__jreviews_media
					SET media_function = CASE media_id
						WHEN " .(int) $mediaId." THEN " . $this->Quote($function) . "
						ELSE (
							IF(media_function = ". $this->Quote($function) . ", '', media_function)
						)
					END
				WHERE
			...
			$query = "
				INSERT INTO #__jreviews_listing_totals
					...
					SELECT
						...
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0), 1, 0)) media_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'video', 1, 0)) video_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'photo', 1, 0)) photo_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'audio', 1, 0)) audio_count,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'attachment', 1, 0)) attachment_count
					FROM
						#__jreviews_media AS Media
					LEFT JOIN
						". EverywhereComContentModel::_LISTING_TABLE . " AS Listing ON Listing." . EverywhereComContentModel::_LISTING_ID . " = Media.listing_id AND Media.extension = 'com_content'
					{$where}
			...
			$query = "
				INSERT INTO #__jreviews_listing_totals
					...
					SELECT
						...
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0), 1, 0)) media_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'video' AND published = 1 AND approved = 1, 1, 0)) video_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'photo' AND published = 1 AND approved = 1, 1, 0)) photo_count_user,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'audio' AND published = 1 AND approved = 1, 1, 0)) audio_count,
						SUM(IF((Media.user_id != Listing." . EverywhereComContentModel::_LISTING_USER_ID . " OR Media.user_id = 0) AND media_type = 'attachment' AND published = 1 AND approved = 1, 1, 0)) attachment_count
					FROM
						#__jreviews_media AS Media
					LEFT JOIN
						". EverywhereComContentModel::_LISTING_TABLE . " AS Listing ON Listing." . EverywhereComContentModel::_LISTING_ID . " = Media.listing_id AND Media.extension = 'com_content'
	...
	protected function getMediaForCmsSync($listing)
	{
		// There are two scenarios. Either we pass the whole listing array and check for MainMedia
		// Or we are already passing the MainMedia, but we don't have the complete listing array

		if(isset($listing['Listing']) && isset($listing['MainMedia']))
		{
			$media = $listing['MainMedia'];
		}
		else {
			$media = $listing['Media'];
		}

		if(Sanitize::getString($media,'extension') != 'com_content'
			||
			!isset($media['media_info'])
		){
			return false;
		}
	...
	function updateListingCoreImage($listing)
	{
		$Config = Configure::read('JreviewsSystem.Config');

		$sync_image = Sanitize::getString($Config,'media_article_sync');

		if($sync_image == 'none' || _CMS_NAME != 'joomla') return;

        $JVersion = new JVersion();

        $cmsVersion = $JVersion->RELEASE;

        $remoteUrlSupport = version_compare($cmsVersion, '3.5', '>=');
		...
		$media = $this->getMediaForCmsSync($listing);

		if(!$media) return;

		$imageUrl = $media['media_info']['image']['url'];

		// Only do this for images stored locally

		if(!strstr($imageUrl, WWW_ROOT) && !$remoteUrlSupport)
		{
			return;
		}
		elseif (strstr($imageUrl, WWW_ROOT)) {
			$imageUrl = str_replace(WWW_ROOT, '', $imageUrl);
		}
		...
		$query = '
			...
			FROM
				#__content
			WHERE
				id = ' . (int) $media['listing_id']
		;

		$images = $this->query($query,'loadResult');
        ...
        $core_image = Sanitize::getString($images, $sync_image);

        $images[$sync_image] = $imageUrl;

        if($core_image == '' || $core_image != $images[$sync_image])
        {
            // Update the article with the main media info

            $query = "
                UPDATE
                    #__content
                SET
                    images = " . $this->Quote(json_encode($images)) . "
                WHERE
                    id = " . (int) $media['listing_id']
                ;
	...
	function updatePostFeaturedImage($listing)
	{
		global $post; // We assume this is the listing detail page which is where the synchronizaton will take place

		$Config = Configure::read('JreviewsSystem.Config');

		$sync_image = Sanitize::getString($Config,'media_article_sync');

		if($sync_image == 'none') return;
		...
		if(!($media = $this->getMediaForCmsSync($listing)))
		{
			return false;
		}

		$mediaUrl = $media['media_info']['image']['url'];
		...
		if($thumbId)
		{
			...
			if(!empty($meta) && basename($featuredImageUrl) == basename($media['media_info']['image']['url']))
			{
				// Check the image size

				// The image name matches, so it's already synched, nothing else to do here
				return false;
			}

			// Delete the existing featured image because we need to replace it

			if($mediaUrl != '')
			{
				wp_delete_attachment($thumbId, $forceDelete = true /* skip trash */);
			}
		}

		global $post;

		// Create local copy of image to associate it with the WP post

		$tmp = download_url( $mediaUrl );

		$post_id = $post->ID;

		$desc = $listing['Listing']['title'];
	    ...
	    preg_match('/[^\?]+\.(jpg|jpe|jpeg|gif|png)/i', $mediaUrl, $matches);
	    ...
	    $thumbId = media_handle_sideload( $fileArray, $post_id, $desc );
	...
	function removeListingCoreImage($media)
	{
		if(_CMS_NAME == 'joomla')
		{
			$this->removeListingCoreImageJoomla($media);
		}
	}

	function removeListingCoreImageJoomla($media)
	{
		$Config = Configure::read('JreviewsSystem.Config');
		...
		$extension = Sanitize::getString($media,'extension');

		$listing_id = Sanitize::getString($media,'listing_id');

		$main_media = Sanitize::getString($media,'main_media');

		if($extension == 'com_content' && $listing_id > 0 && $main_media == 1)
		{
			$query = '
				SELECT
					images
	...
	function completeMediaUrls($media, $filename_url)
	{
		...
		if(isset($media_info['thumbnail']) && !empty($media_info['thumbnail']))
		{
			...
			{
				$url_parts = parse_url($filename_url);

				extract($url_parts); /* $scheme, $host, $path */
				...
				if(isset($scheme) && isset($host)) {
					...
					$tn_url = $scheme . '://' . $host . ($port ? ':' . $port : '') . $path;
				}
				else {
	...
	function getListingInfo($media_id)
	{
		$query = '
			SELECT
				listing_id, review_id, extension, Listing.' . EverywhereComContentModel::_LISTING_USER_ID . ' AS owner_id
			FROM
				#__jreviews_media AS Media
			LEFT JOIN
				' . EverywhereComContentModel::_LISTING_TABLE . ' AS Listing ON Listing.id = Media.listing_id
			WHERE
...
//		}
        ...
        foreach($results AS $key=>$row)
        {
			...
			if($modelName == 'Listing' && (
					($controller == 'com_content' && $action == 'com_content_view')
					||
...
//						tmp.rank_audio <= {$audio_limit}
			...
			foreach($media AS $media_id=>$row)
			{
				...
				if($row['media_function'] == 'logo' && $modelName == 'Listing')
				{
					$mediaArray[$object_id]['Logo'] = $row;

					$logoMedia = $row;
				}

				if($row['media_function'] == 'cover' && $modelName == 'Listing')
				{
					$mediaArray[$object_id]['Cover'] = $row;
				}
				...
				elseif(in_array($row['media_type'],array('video','photo')) && $controller == 'module_reviews' && $modelName == 'Review' && count($mediaArray) == 0) {
					...
					// Logo and cover automatically excluded from gallery

					if(!in_array($row['media_function'], array('logo','cover')))
					{
			...
			// Need to do another pass to add the logo back to the gallery if there is no cover image and empty covers are disabled

			if($logoMedia && !isset($mediaArray[$object_id]['Cover']) && !$ConfigComponent->listing_detail_header_empty_cover)
			{
				$mediaArray[$object_id]['Media'][$logoMedia['media_type']][] = $logoMedia;
			}
			...
			foreach($results AS $key=>$row)
            {
				...
				if(!isset($results[$key]['MainMedia']['media_info'])
					...
					// Uncommenting will allow default images to appear in the listing detail page even after a review photo/video has been added.
					// It will also force the default image to appear in the listings/one widgets
					// && !isset($results[$key]['Media'])
				) {
	...
	{
		$media_id = Sanitize::getInt($media['Media'],'media_id');

		$listing_id = Sanitize::getInt($media['Media'],'listing_id');

		if($media['Media']['main_media'] == 1)
		{
			$query = "
				UPDATE #__jreviews_media
					SET main_media = 0
		...
		if($this->query($query))
		{
			$this->updateListingCoreImage($media);
