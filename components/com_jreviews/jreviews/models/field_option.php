...
class FieldOptionModel extends MyModel  {
    ...
    var $fields = array(
        ...
        'FieldOption.default AS `FieldOption.default`',
        'FieldOption.description AS `FieldOption.description`',
        'FieldOption.control_field AS `FieldOption.control_field`',
        'FieldOption.control_value AS `FieldOption.control_value`',
        'Field.name AS `FieldOption.name`',
    ...
    * Returns an array of field options for the selected parent field to use as control field values in forms
    */
    function getControlList($fieldid, $valueq, $params = array())
    {
        $defaults = array('show_value' => false);
        ...
        if(Sanitize::getBool($params, 'show_value', false))
        {
            $textColumn = 'CONCAT(text, " (", value, ")")';
        }

        $query = "
            SELECT
                optionid, value, $textColumn AS label
            FROM
                #__jreviews_fieldoptions
            WHERE
                fieldid = {$fieldid}
        ...
        $rows = $this->query($query, 'loadObjectList', 'optionid');
        ...
        $rows_second = $this->query($query, 'loadObjectList', 'optionid');
    ...
    function save(&$data)
    {
        ...
        $FieldOptionToField = $this->query($query, 'loadResult');

        if($FieldOptionToField == '' && !empty($control_value))
        {
