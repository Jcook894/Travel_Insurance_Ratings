...
class CronFunctionsComponent extends S2Component {
    ...
    */
    ...
    {
		$cron_period = Sanitize::getVar($this->c->Config,'ratings_rebuild_interval') * 3600;

		$last_run = Sanitize::getInt($this->c->Config,'ratings_rebuild_last');

		$now = time();

		if($last_run + $cron_period <= $now)
		{
			$this->c->Config->store(array('ratings_rebuild_last' => $now));

			S2App::import('Model','review','jreviews');

			$Review = ClassRegistry::getClass('ReviewModel');

			$Review->updateBayesianAverages();
		}
    }
