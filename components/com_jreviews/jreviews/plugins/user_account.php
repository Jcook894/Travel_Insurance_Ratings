...
/**
 * Automatically creates user accounts for guest submissions.
**/
...
class UserAccountComponent extends S2Component {
    ...
    */
    var $controllerActions = array(
        ...
        'listings'=>array('detail','create','_loadForm','_save'),
    ...
    function startup(&$controller)
    {
        ...
        if($User->id > 0
            || (Sanitize::getInt($user_session,'user_id') == 0 && Sanitize::getString($user_session,'name') == '' && Sanitize::getString($user_session,'email') == '')
     ...
     */
    function plgBeforeBeforeSave(&$model, $data)
    {
        $user_id_key = false;
        ...
        switch($model->name)
        {
            ...
            case 'Listing':
                ...
                if(!Sanitize::getInt($data['Listing'],'id')) {
                    ...
                    // Not using the EverywhereComContentMode::_LISTING_USER_ID constant because this runs before the 'beforeSave' method which
                    // expects to receive the normalized 'created_by' key
    ...
    function coreRegistration(&$model, $data, $user_id_key)
    {
        ...
        if(empty($user_session)) {
            ...
            $valid_email = Sanitize::getBool($data,'valid_email');

            if($username && $valid_username && $email && $valid_email && Sanitize::getInt($data[$model->name],$user_id_key) == 0)
            {
                $user = $this->c->User->findRow(array('conditions'=>array(
                    'User.' . UserModel::_USER_ALIAS . ' = ' .$this->c->Quote($username) . ' OR ' . UserModel::_USER_EMAIL . ' = ' . $this->c->Quote($email)
                )));
                ...
                $user_data = array(
                    ...
                    'email'=>$email,
                    'password'=>null,
                    'activation'=>cmsFramework::getUserConfig('activation')
                );
    ...
    static function getUser($session_only = true)
    {
        ...
        $user = $Registration->findRow(array(
                'fields'=>array('Registration.user_id','Registration.name','Registration.email','User.' . UserModel::_USER_ALIAS . ' AS `Registration.username`'),
                'joins'=>array('LEFT JOIN ' . UserModel::_USER_TABLE . ' AS User ON User.' . UserModel::_USER_ID . ' = Registration.user_id'),
                'conditions'=>array(
