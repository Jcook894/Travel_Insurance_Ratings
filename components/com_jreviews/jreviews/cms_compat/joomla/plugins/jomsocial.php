...
class JomsocialComponent extends S2Component {
    ...
    function startup(&$controller)
    {
        ...
        if($version < 3 || $this->c->Config->community != 'jomsocial')
        {
    ...
    function plgAfterSave(&$model)
    {
        ...
        if($this->points)
        {
            require_once( PATH_ROOT . 'components' . DS . 'com_community' . DS . 'libraries' . DS . 'userpoints.php');
        }
    ...
    function _plgListingAfterSave(&$model)
    {
		...
		if(!$skip_points && $this->points && isset($model->isNew) && $model->isNew && $listing['Listing']['state'] == 1 && !$this->_isPaidListing($listing))
        {
            // Begin add points
    ...
    function _plgMediaBeforeDelete(&$model)
    {
        ...
        // Begin deduct points
        if($this->points && $media['Media']['published'] == 1 && $media['Media']['approved'] == 1 && $media['Media']['user_id'] > 0)
        {
            CuserPoints::assignPoint('jreviews.media.delete',$media['Media']['user_id']);

            CuserPoints::assignPoint('jreviews.media.'.$media_type.'.delete',$media['Media']['user_id']);
        }
    }
    ...
    function _plgMediaAfterSave(&$model)
    {
        ...
        if(!$this->inAdmin || ($this->inAdmin && Sanitize::getBool($model->data,'moderation')))
        {
                ...
                && Sanitize::getInt($model->data['Media'],'approved') == 1)
            {
                ...
                $curr_media = $media = $model->findRow(array('conditions'=>array('Media.media_id = ' . $media_id)),array());
                ...
                if($user_id == 0) {
                    ...
                    $today = date('Y-m-d',strtotime(_TODAY));

                    $conditions[] = '(
                            DATE(Media.created) = "'. $today .'"
                                || DATE(Media.modified) = "'. $today .'"
                                || Media.media_id = '. $media_id .'
                        )';
                }

                $time = '00:'.str_pad($this->aggregation_period,2,0).':00';

                $conditions[] = "
                    (Media.created BETWEEN SUBTIME(" . $this->c->Quote( _CURRENT_SERVER_TIME ) . ", " . $this->c->Quote($time) . ")
                    AND ADDTIME(" . $this->c->Quote( _CURRENT_SERVER_TIME ) . ", " . $this->c->Quote($time) . "))";

                $queryData = array(
                    'conditions'=>$conditions,
                    'order'=>array('Media.created DESC'),
                    'limit'=>5 // The number of photos displayed on the stream per user will be limtied to 5
                    );
                ...
                if(!in_array($media_type,array('attachment','audio')))
                {
                    foreach($media AS $m)
                    {
                        $thumb_src = $this->Media->thumb($m,array(
                            'thumbnailer'=>'api',
                            'size'=>$this->tn_size,
                            'mode'=>$this->tn_mode,
                            'return_thumburl'=>true,
                        ...
                        $media_url = $this->Routes->mediaDetail('', array('media'=>$m, 'listing'=>$listing), array('return_url'=>true));
                ...
                if($act['actor'] > 0 && count($mediaArray) > 0)
                {
                    $query  = "
                        ...
                        WHERE
                            ...
                            AND cid = " . $act['cid'] . "
                            AND actor = " . $act['actor'] . "
                            AND created BETWEEN SUBTIME(" . $this->c->Quote( _CURRENT_SERVER_TIME ) . ", " . $this->c->Quote($time) . ")
                            AND ADDTIME(" . $this->c->Quote( _CURRENT_SERVER_TIME ) . ", " . $this->c->Quote($time) . ")"
                    ;
        ...
        if($this->points)
        {
                ...
                && Sanitize::getInt($model->data['Media'],'approved') == 1)
            {
                ...
                CuserPoints::assignPoint('jreviews.media.add',$user_id);

                CuserPoints::assignPoint('jreviews.media.'. $media_type . '.add',$user_id);
            }
        }
    }
    ...
    function streamPost($act, $params)
    {
        ...
        if($act->actor > 0)
        {
            ...
            $profileprivacy = $UserParams->get('privacyProfileView');
        }
        else {
