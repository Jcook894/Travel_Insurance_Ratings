...
/**  Usage
    ...
    2. Click "New" to create a new rule:
        ...
        For "Unique function name" use one of the following options:
            ...
            plgaup_jreviews_media_add
            plgaup_jreviews_media_delete
            plgaup_jreviews_photo_add
            plgaup_jreviews_photo_delete
            plgaup_jreviews_video_add
            plgaup_jreviews_video_delete
            plgaup_jreviews_attachment_add
            plgaup_jreviews_attachment_delete
            plgaup_jreviews_audio_add
            plgaup_jreviews_audio_delete
...
class AlphauserpointsComponent extends S2Component {
	...
	function _plgMediaBeforeDelete(&$model)
	{
        ...
        if(Sanitize::getInt($media['Media'],'published') == 1 && Sanitize::getInt($media['Media'],'approved') == 1)
        {
			$aupid = AlphaUserPointsHelper::getAnyUserReferreID($media['Media']['user_id']);

			if($aupid)
			{
				AlphaUserPointsHelper::newpoints('plgaup_jreviews_media_delete', $aupid);

				$media_type = Sanitize::getString($media['Media'],'media_type');

				if($media_type)
				{
					AlphaUserPointsHelper::newpoints('plgaup_jreviews_' . $media_type . '_delete', $aupid);
				}
			}
        }
	}
	...
	function _plgListingAfterSave(&$model)
	{
		...
		if(!$skip_points && isset($model->isNew) && $model->isNew && $listing['Listing']['state'] == 1 && !$this->_isPaidListing($listing))
		{
			// Begin add points
	...
	function _plgMediaAfterSave(&$model)
	{
        ...
        if($user_id
        	&& (isset($model->isNew) || (!$this->inAdmin && Sanitize::getInt($model->data,'finished') == 1))
        	&& Sanitize::getInt($media['Media'],'published') == 1
        	&& Sanitize::getInt($media['Media'],'approved') == 1)
        {
			$aupid = AlphaUserPointsHelper::getAnyUserReferreID($user_id);

			if($aupid)
			{
				AlphaUserPointsHelper::newpoints('plgaup_jreviews_media_add', $aupid);

				$media_type = Sanitize::getString($media['Media'],'media_type');

				if($media_type)
				{
					AlphaUserPointsHelper::newpoints('plgaup_jreviews_' . $media_type . '_add', $aupid);
				}
			}
        }
	}
