...
class CommunityModel extends MyModel  {
    ...
    var $profileUrl = 'index.php?option=com_comprofiler&amp;view=userprofile&amp;user=%s&amp;Itemid=%s';

    var $registerUrl = 'index.php?option=com_comprofiler&amp;view=registers&amp;Itemid=%s';
    ...
    function __construct(){
        ...
        if (file_exists(PATH_ROOT . 'components' . _DS . 'com_comprofiler' . _DS . 'comprofiler.php'))
        {
            ...
            $this->menu_id = $Menu->getComponentMenuId('com_comprofiler&view=userprofile', true);

            $register_menu_id = $Menu->getComponentMenuId('com_comprofiler&view=registers',true);

            // Compatibility with CB 1.x

            if(!$this->menu_id)
            {
                $this->profileUrl = 'index.php?option=com_comprofiler&amp;task=userProfile&amp;user=%s&amp;Itemid=%s';

                $this->registerUrl = 'index.php?option=com_comprofiler&amp;task=registers&amp;Itemid=%s';

                $register_menu_id = $Menu->getComponentMenuId('com_comprofiler&task=registers',true);
            ...
            Configure::write('Community.register_url',cmsFramework::route(sprintf($this->registerUrl,$register_menu_id)));
        }
