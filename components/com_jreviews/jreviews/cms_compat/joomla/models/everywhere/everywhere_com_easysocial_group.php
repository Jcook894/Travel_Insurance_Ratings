...
class EverywhereComEasysocialGroupModel extends MyModel  {

    const _LISTING_ID = 'id';

	var $UI_name = 'EasySocial - Groups';
	 ...
	 * @var string
	 */
	var $extension = 'com_easysocial_group';
	 ...
	 * @var string
	 */
	var $extension_alias = 'com_easysocial_group';
	...
	var $fields = array(
		...
		"'com_easysocial_group' AS `Listing.extension`",
    ...
    var $conditions = array('Listing.cluster_type = "group"');
     ...
     */
    var $joins = array(
        'Totals'=>"LEFT JOIN #__jreviews_listing_totals AS Totals ON Totals.listing_id = Listing.id AND Totals.extension = 'com_easysocial_group'",
        'LEFT JOIN #__social_clusters_categories AS Category ON Listing.category_id = Category.id AND Category.type = "group"',
        "INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Category.id AND JreviewsCategory.`option` = 'com_easysocial_group'",
        'LEFT JOIN #__social_avatars AS Avatar ON Listing.id = Avatar.uid AND Avatar.type = "group"'
    );
    ...
    var $joinsReviews = array(
        'LEFT JOIN #__social_clusters AS Listing ON Review.pid = Listing.id AND Listing.cluster_type = "group"',
        'LEFT JOIN #__social_clusters_categories AS Category ON Listing.category_id = Category.id AND Category.type = "group"',
        "INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Category.id AND JreviewsCategory.`option` = 'com_easysocial_group'"
    );

    // var $joinsMedia = array(
    //     'LEFT JOIN #__social_clusters AS Listing ON Media.listing_id = Listing.id AND Listing.cluster_type = "group" AND Media.extension = "com_easysocial_group"',
    //     'LEFT JOIN #__social_clusters_categories AS Category ON Listing.category_id = Category.id AND Category.type = "group"',
    //     "INNER JOIN #__jreviews_categories AS JreviewsCategory ON JreviewsCategory.id = Category.id AND JreviewsCategory.`option` = 'com_easysocial_group'"
    // );
    ...
    var $default_thumb  = 'media/com_easysocial/defaults/avatars/group/large.png';

    var $avatar_path = 'media/com_easysocial/avatars/group/';
	...
	function __construct()
	{
        ...
        $this->tag = __t("COMMUNITY_GROUP_TAG",true);  // Used in MyReviews page to differentiate from other component reviews
	...
	function listingUrl($listing)
	{
        ...
        $groups = $groupsModel->getGroups(array('inclusion' => $listing['Listing']['listing_id']));

        $group = array_shift($groups);

        $url = ESR::groups( array( 'layout' => 'item' , 'id' => $group->getAlias()  , 'appId' => $this->easysocialApp['id'] . '-' . $this->easysocialApp['alias'] , 'sef' => false) , false );
