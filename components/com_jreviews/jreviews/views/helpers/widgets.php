...
class WidgetsHelper extends MyHelper
{
    function __construct()
    {
        ...
        $this->CustomFields = ClassRegistry::getClass('CustomFieldsHelper');
    }
    ...
    {
        if(!$this->Access->canSeeReviewButton($listing['User']['user_id']))
        {
            return;
        }
        ...
        if(!$listing['User']['duplicate_review'] && Sanitize::getInt($listing['Criteria'],'state') == 1 && $this->Config->user_reviews):?>

            <div class="jr-review-add jr-listing-info jrButton jrSmall"><span class="jrIconAddReview"></span><?php JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_review_write_detail'));?></div>
    ...
    function download($attachment)
    {
        ...
        $integrity_token = cmsFramework::getCustomToken((int) $media_id, $media_type, $filename, $created);
            ...
            if($recipient_exists) {
                ...
                ?>
                    ...
                    <button class="jr-send-inquiry jrButton jrSmall" data-listing-id="<?php echo $listing_id;?>">
                        ...
                        <span class="jrIconMessage"></span><span><?php JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_listing_send_inquiry'));?></span>
    ...
    {
        $review_id = (int) $review['Review']['review_id'];
        ...
        <div class="jrReviewHelpful">
            ...
            <div class="jr-review-vote jrButtonGroup jrLeft" data-review-id="<?php echo $review_id;?>" data-state="<?php echo $state;?>">
                ...
                <button href="#review-vote" class="jrVoteYes jrButton jrSmall" data-vote="yes">
                        ...
                        <span class="jrButtonText"><?php echo $review['Vote']['yes'];?></span>
                ...
                <button href="#review-vote" class="jrVoteNo jrButton jrSmall" data-vote="no">
                        ...
                        <span class="jrButtonText"><?php echo $review['Vote']['no'];?></span>
    ...
    function relatedListingsJS($listing)
    {
        ...
        $related_reviews = Sanitize::getVar($listing['ListingType']['config'],'relatedreviews',array());

        $related_reviews = array_filter($related_reviews);
        ...
        foreach($related_reviews AS $key=>$related_review):

            if(!Sanitize::getInt($related_review,'enable',0)) continue;

            $module_id = 10000 + $listing_id + $key;

            $target_id = Sanitize::getString($related_review,'target_id','jrRelatedListings');

            $target_class = Sanitize::getString($related_review,'target_class');

            extract($related_review);

            $title = str_ireplace('{title}',$listing_title,__t(Sanitize::getString($related_review,'title'),true));
    ...
    */
    function discussionManager($post)
    {
        ...
        $token = cmsFramework::getCustomToken((int) $discussion_id);
    ...
    function listingManagerLinks($listing)
    {
        $this->listingManager($listing, $dropdownMenu = false);
    }

    function listingManager($listing, $dropdownMenu = true)
    {
        ...
        // Get additional options added via add-ons

        $menuExtras = Configure::read('widget_listing_manager',array());

        $menuExtras = Sanitize::getVar($menuExtras,$listing_id,array());
        ...
        <?php endif;?>
            ...
            <ul class="jr-listing-manager jrDropdownMenu">
                ...
                <?php if(isset($menuExtras[1])):?>

                    <?php foreach($menuExtras[1] AS $extra):?>
                ...
                <li>
                    ...
                    <?php echo $this->Routes->listingEdit($icon . ' <span>' . JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_listing_edit'), true) . '</span>',$listing,array('class'=>'jr-edit'));?>
                </li>

                <?php endif;?>

                <?php if(isset($menuExtras[2])):?>

                    <?php foreach($menuExtras[2] AS $extra):?>
                ...
                <?php if(isset($menuExtras[3])):?>

                    <?php foreach($menuExtras[3] AS $extra):?>
                ...
                <?php if(isset($menuExtras[4])):?>

                    <?php foreach($menuExtras[4] AS $extra):?>
                ...
                <?php if(isset($menuExtras[5])):?>

                    <?php foreach($menuExtras[5] AS $extra):?>
                ...
                <?php if(isset($menuExtras[6])):?>

                    <?php foreach($menuExtras[6] AS $extra):?>
                ...
                <?php if(isset($menuExtras[7])):?>

                    <?php foreach($menuExtras[7] AS $extra):?>
    ...
    function photoFunctionSelection($listing, $media)
    {
        if($this->Access->isGuest()) return;

        $logo = $this->Config->media_photo_function_logo;

        $cover = $this->Config->media_photo_function_cover;

        if(!$logo && !$cover) return;

        extract($media['Media']);

        if($this->Access->canManageMedia($media_type, $user_id, $listing['Listing']['user_id']) && $media_type == 'photo' && $review_id == 0)
        {
            $formTokenKeysEdit = array('media_id'=>'media_id','listing_id'=>'listing_id','review_id'=>'review_id','extension'=>'extension','user_id'=>'user_id');
            ...
            <div class="jr-photo-function jrButtonGroup" data-media-id="<?php echo $media_id;?>" data-token="<?php echo $formToken;?>" data-state-class='{"gallery":"jrBlue","logo":"jrGreen","cover":"jrOrange"}'>

                <button class="jrButton <?php echo $media_function == '' ? 'jrBlue' : '';?>" data-function="gallery">

                    <span><?php echo JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_media_photo_function_gallery'));?></span>

                </button>

                <?php if($logo):?>

                    <button class="jrButton <?php echo $media_function == 'logo' ? 'jrGreen' : '';?>" data-function="logo">

                        <span><?php echo JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_media_photo_function_logo'));?></span>
                ...
                <?php if($cover):?>
                    ...
                    <button class="jrButton <?php echo $media_function == 'cover' ? 'jrOrange' : '';?>" data-function="cover">

                        <span><?php echo JreviewsLocale::getConstant(Sanitize::getString($this->Config, 'lang_media_photo_function_cover'));?></span>
    ...
    function mediaSetMain($listing, $media)
    {
        ...
        if($canSetMainMedia) {
            ?>
                ...
                <button class="jr-media-main jrButton"

                    data-main-media = "<?php echo $main_media ? 1 : 0;?>"
    ...
    function listingDetailButtons($listing)
    {
        ...
        $menuExtras = Configure::read('widget_listing_buttons_detail',array());
        ...
        <?php
            ...
            if(isset($menuExtras[1]))
            {
                foreach($menuExtras[1] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[2]))
            {
                foreach($menuExtras[2] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[3]))
            {
                foreach($menuExtras[3] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[4]))
            {
                foreach($menuExtras[4] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[5]))
            {
                foreach($menuExtras[5] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[6]))
            {
                foreach($menuExtras[6] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[7]))
            {
                foreach($menuExtras[7] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[8]))
            {
                foreach($menuExtras[8] AS $extra)
                {
                    echo $extra;
                }
            }
            ...
            if(isset($menuExtras[9]))
            {
                foreach($menuExtras[9] AS $extra)
                {
                    echo $extra;
                }
            }
    ...
    function listPageButtons($listing, $mobile = false) {
        ...
        $menuExtras = Configure::read('widget_listing_buttons_list',array());
        ...
        if($user_reviews && $this->Config->list_show_readreviews && Sanitize::getInt($listing['Criteria'],'state') == 1) {
        ...
        if($user_reviews && $this->Config->list_show_readreviews && Sanitize::getInt($listing['Criteria'],'state') == 2) {
        ...
        if($user_reviews && $this->Config->list_show_newreview && Sanitize::getInt($listing['Criteria'],'state') == 1) {
