...
class sef_jreviews {
	...
	function &getInstance() {
		...
		if (!isset($instance[0]) || !$instance[0]) {
			...
			$instance[0]->joomla_style_params = 1;
	...
	function create($string)
	{
		...
		$query_params = sef_jreviews::buildUrlParams($params, $_this->joomla_style_params);

		if($query_params && ((!empty($params) && empty($query_string)) || (!empty($query_string) && $query_string[0] != 'tag'))) {

			if($query_params)
			{
				$sefstring .= '/?' . $query_params;
			}
		}
		if($query_params && ((!empty($params) && empty($query_string)) || (!empty($query_string) && $query_string[0] == 'tag'))) {
