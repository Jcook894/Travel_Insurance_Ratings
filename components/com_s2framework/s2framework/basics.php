...
require( S2_LIBS . 'cms_compat' . DS . _CMS_NAME . DS . 'framework.php' );
require( S2_FRAMEWORK . DS . 'dispatcher.php' );

// Transition to namespacing for autoloading of vendor libraries

require( S2_VENDORS . 'autoload.php');
...
$now_local = cmsFramework::dateUTCToLocal($now);

$DateTime = new DateTime($now_local);

$_BEGINNING_OF_TODAY = cmsFramework::dateLocalToUTC($DateTime->format('Y-m-d 00:00:00'));

$DateTime->add(new DateInterval('P1D'));

$_END_OF_TODAY = cmsFramework::dateLocalToUTC($DateTime->format('Y-m-d 00:00:00'));

!defined('_TODAY') and DEFINE('_TODAY', $_BEGINNING_OF_TODAY);

!defined('_END_OF_TODAY') and DEFINE('_END_OF_TODAY', $_END_OF_TODAY);
...
/**
* Generate a cache key that is different for different domains/paths and locale
**/
function S2CacheKey($base, $var = '')
{
	$var = is_array($var) ? serialize($var) : $var;

	$https = isset($_SERVER['HTTPS']) ? 'https' : '';

    $cache_key = strtolower(str_replace('-','_',cmsFramework::locale())) . '_' . $base . '_' . md5(WWW_ROOT . PATH_ROOT . cmsFramework::getConfig('secret') . $https . $var);
...
function s2_num_format($number, $decimals = 2)
{
    return number_format($number,$decimals,__l('DECIMAL_SEPARATOR',true),__l('THOUSANDS_SEPARATOR',true));
}
...
class s2Messages
{
    ...
    public static function errorGeneric()
    {
        return __t("There was a problem processing the request.",true);
    }
