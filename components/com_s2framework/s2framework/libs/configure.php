 ...
 * @access public
 */
    static function getInstance($app = false) {

        static $instance = array();

        if (!$instance) {
            $instance[0] = new Configure();
            $instance[0]->__loadBootstrap($app);
        }
    ...
    function listS2Objects($type, $path = null, $cache = true) {
        ...
        if (empty($_this->__S2Objects) || !isset($_this->__S2Objects[$type]) || $cache !== true) {
            $types = array(
                ...
                'job' => array('suffix' => '.php', 'base' => null),
 ...
 */
    function load($fileName) {
        ...
        if (!isset($config)) {
            $error = "Configure::load() - no variable \$config found in %s.php";
 ...
 */
    function __loadBootstrap($app)
    {
        ...
        if(!$fileArray = S2Cache::read($cache_key,'_s2framework_core_'))
        {
            ...
            if (!file_exists($configPath . 'paths.php')) {
                trigger_error(sprintf("Can't find application paths file. Please create %spaths.php, and make sure it is readable by PHP.", $configPath), E_USER_ERROR);
            }

            include($configPath . 'paths.php');

            $this->basePaths = $basePaths;

            $fileArray = $this->buildPaths($relativePaths);

            // Build the locale array
            ...
            foreach($basePaths AS $basePath)
            {
                ...
                {
                    $tree = $Folder->tree($basePath . 'locale', array('index.html'), 'files');

                    foreach($tree AS $path)
                    {
                        $path = str_replace(PATH_ROOT, '', $path);

                        if(strstr($path, 'LC_MESSAGES'))
                        {
                            $path = str_replace('\\', '/',$path);

                            if(preg_match('/(?P<path>[\w].*\/locale)\/(?P<lang>[\w].*)\/LC_MESSAGES\/(?P<domain>[\w].*).po/', $path, $matches))
                            {
                                $files[$matches['lang']]['LC_MESSAGES'][$matches['domain']][] = $matches['path'];
                            }
                        }
                    }

                    $files['paths'][] = str_replace(PATH_ROOT, '', $basePath) . 'locale';
                }
            }

            $fileArray['Locale'] = $files;

            $Folder = new S2Folder();

            $items = array();

            $default_processed = false;
            ...
            foreach($basePaths AS $basePath)
            {
                if(false === strstr($basePath,'s2framework')) // Ignore the s2framework folder
                {
                    ...
                    if(!empty($dirs))
                    {
                        ...
                        {
                            if($theme == 'default' && !$default_processed)
                            {
                            ...
                            {
                                if(strstr($basePath,PATH_APP_ADDONS)) {
                                    $items['Theme'][$theme]['.info']['location'] = 'addon';
                                }
                                elseif(strstr($basePath,PATH_APP)) {
                                    $items['Theme'][$theme]['.info']['location'] = $app;
                                }
                                 elseif(strstr($basePath,PATH_APP_OVERRIDES)) {
                                    $items['Theme'][$theme]['.info']['location'] = 'overrides';
                                }

                                if($theme == 'default' && $items['Theme'][$theme]['.info']['location'] == 'overrides') continue;
                                ...
                                foreach($tree[1] AS $file)
                                {
                                    ...
                                    if(count(explode(DS,$path))>2)
                                    {
                                        ...
                                        if($items['Theme'][$theme]['.info']['location'] == $app)
                                        {
                                        ...
                                        if(
                                                $items['Theme'][$theme]['.info']['location'] != $app // Not in component folder
                                                &&
                                                !isset($items['Theme'][$fallback_theme][$theme_folder][$basename]) // Not in default folder
                                                &&
                                                $basename != 'custom_styles.css' // Exclude custom_styles which only loads from the selected theme
                                        ) {
 ...
 */
    static function import($type = null, $name = null, $app = 'jreviews')
    {
        ...
        if(is_array($name)){
            ...
            foreach($name AS $n){
                $res = $res && S2App::import($type,$n,$app);
            }
 ...
 */
    function __paths($type, $name, $app) {
        ...
        switch ($type) {
            ...
            case 'admin_job':
             $paths = array(
                 $overrides . 'admin_jobs' . DS ,
                 $app_path . 'admin_jobs' . DS ,
                 $app_path);
            break;
            case 'component':
            case 'components':
             $paths = array(
                 ...
                 $overrides . 'repositories' . DS ,
                 $app_path . 'controllers' . DS . 'components' . DS,
                 $app_path . 'repositories' . DS,
                 $app_path);
            break;
            case 'job':
             $paths = array(
