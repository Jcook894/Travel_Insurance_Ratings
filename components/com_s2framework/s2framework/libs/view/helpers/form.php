...
class FormHelper extends MyHelper {
    ...
    function label($text = null, $attributes = array()) {
...
//            $label = false;
        ...
        if ($label !== false) {
            ...
            if (isset($options['id'])) {
                ...
                $out = $this->label($labelText, $labelAttributes); // Moved inside if statement in 2.0.9

            } else {

                $labelAttributes = $labelAttributes;

                $out = $this->label($labelText, $labelAttributes);
            }
        }
...
/*        if ($type != 'hidden') {
    ...
    function select($fieldName, $options = array() , $selected = null, $attributes = array())
    {
        ...
        $optionDataByKey = Sanitize::getVar($attributes, 'option_attr_by_key', array());

        unset($attributes['option_attr_by_key']);
        ...
        foreach ($options as $value => $text)
        {
            ...
            if(is_array($text) || is_object($text))
            {
                ...
                if (!empty($optionDataByKey))
                {
                    foreach ($optionDataByKey AS $data)
                    {
                        $dataAttr = $data['attr'];

                        $valueKey = $data['key'];

                        if (isset($text[$valueKey]))
                        {
                            $optionAttr[] = $dataAttr .'=' .$text[$valueKey];
                        }
                    }
                }
            ...
            {
                $optionAttr[] = (deep_in_array($value,$selected,true) ? ' selected="selected"' : '');

            # Single select list
            } elseif ($selected) {

                $optionAttr[] = ($value == $selected) ? ' selected="selected"' : '';
            ...
            $disabled and $optionAttr[] = 'disabled="disabled"';

            $select .= sprintf($this->Html->tags['selectoption'], $value, implode(' ',$optionAttr), $text);
        }
    ...
    function checkbox($fieldName, $options = array(), $attributes = array())
    {
        ...
        if(is_array($options))
        {
            foreach ($options as $value => $text)
            {
                ...
                if (Sanitize::getInt($attributes,'data-search'))
                {
                    $tagName .= '-search';
                }
                ...
                if($label)
                {
                    $labelAttr = $label && is_array($label) && !empty($label) ? ' '. $this->Html->_parseAttributes($label) : '';
                    ...
                    $optionLabel = $this->label($text, array('for'=>$tagName));
                    $checkbox = sprintf($this->Html->tags['checkboxmultiple'], $fieldName, $tagName, $parsedOptions, $optionLabel);
        ...
        if(is_array($options))
        {
            foreach ($options as $value => $text)
            {
                ...
                if ($label) {
                    ...
                    $checkbox = sprintf($this->Html->tags['checkbox'], $fieldName, $tagName, $parsedOptions, $optionLabel);
    ...
    function radio($fieldName, $options = array(), $attributes = array()) {
        ...
        foreach ($options as $optValue => $optTitle) {
            ...
            if($label_wrap)
            {
                $input =  sprintf($this->Html->tags['radio'], $fieldName, $tagName, $parsedOptions, $optTitle );

                $radio = $this->label($input, array('for'=>$tagName,'class'=>$fieldOptionClass));
            }
            else {

                $optionLabel = $this->label($optTitle, array('for'=>$tagName));

                $radio =  sprintf($this->Html->tags['radio'], $fieldName, $tagName, $parsedOptions, $optionLabel );
            }
