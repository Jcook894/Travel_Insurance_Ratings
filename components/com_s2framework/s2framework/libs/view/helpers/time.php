 ...
 */
	function nice($date_string = null, $format = null, $offset = null) {
		...
		if(is_null($offset))
        {
			$date_string = cmsFramework::dateUTCtoLocal($date_string);
		}
 ...
 */
    function timeAgoInWords($dateTime, $options = array())
    {
        ...
        if ($diff > abs($now - $this->fromString($end))) {
            ...
            $relativeDate = sprintf(__t('on %s',true), $this->format($inSeconds, $format));
        }
        else {
            if ($years > 0) {
 ...
 */
	...
	function format($date, $format = '%B %d, %Y') {
		...
		elseif($format == 'd F, Y')
            ...
            if(!strstr($format, '%')) {
                $format = str_replace(array('F', 'd', 'Y'), array('%B', '%d', '%Y'), $format);
            }
        }
