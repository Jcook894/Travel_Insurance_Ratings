 ...
 */
	function __bindTextDomain($domain)
	{
		...
		$appPaths = $this->app . 'Paths';

		$fileArray = $S2App->{$appPaths};

        $searchPaths = $fileArray['Locale'];

		$langPath = $this->l10n->languagePath;

		foreach($searchPaths['paths'] as $directory)
		{
			$translations = array();

			foreach ($langPath AS $lang)
			{
				$file = PATH_ROOT . $directory . DS . $lang . DS . $this->category . DS . $domain;

				$default = PATH_ROOT . $directory . DS . 'eng' . DS . $this->category . DS . $domain;

				$override = PATH_ROOT . 'templates/jreviews_overrides/locale/' . $lang . DS . $this->category . DS . $domain;

				// Load the default english file

				if(file_exists("$default.po"))
				{
					$df = fopen("$default.po", "r");

					$translations = array_filter($this->__loadPo($df, $domain, false));
				}
				...
				if(file_exists("$file.po"))
				{
					...
					$translations = array_replace($translations, array_filter($this->__loadPo($f, $domain, false)));
				}

				// Load the override locale file

				if(file_exists("$override.po"))
				{
					$of = fopen("$override.po", "r");

					$translations = array_replace($translations, array_filter($this->__loadPo($of, $domain, false)));
				}
			}

			// Combine all three locale files with replacements

			if(!empty($translations))
			{
				if(isset($this->__domains[$this->category][$this->__lang][$domain])
					&& is_array($this->__domains[$this->category][$this->__lang][$domain]))
				{
					$currTranslations = $this->__domains[$this->category][$this->__lang][$domain];

					$this->__domains[$this->category][$this->__lang][$domain] = array_replace($currTranslations, $translations);
				}
				else {

					$this->__domains[$this->category][$this->__lang][$domain] = $translations;
				}
			}
		}
		...
		if (isset($this->__domains[$this->category][$this->__lang][$domain]['']) && $head = $this->__domains[$this->category][$this->__lang][$domain][''])
		{
			foreach (explode("\n", $head) as $line)
			{
				$header = strtok($line,":");
 ...
 * @access private
 */
	function __loadPo($file, $domain, $set_global = true)
	{
		$type = 0;
		$translations = array();
		$translationKey = "";
		...
		$result = array_merge($merge ,$translations);

		if($set_global)
		{
			$this->__domains[$this->category][$this->__lang][$domain] = $result;
		}
