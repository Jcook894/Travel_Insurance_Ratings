	...
	if (strpos($target, "http://") === false && strpos($target, "https://") === false) { // check for a http link which means web service
		...
		if ($lpos === false) {
			...
			if($messageType == 'flex.messaging.messages.RemotingMessage')
			{
				...
				$uriclasspath = str_replace('.',DS,$classAndPackage) . '.php';
			...
			if(!$handled)
			{
				$uriclasspath = "amfphp".DS."Amf3Broker.php";
				$classpath = $baseClassPath . "amfphp".DS."Amf3Broker.php";
			...
			if ($lpos === false) {
				...
				if ($classname == "PageAbleResult" && $methodname == 'getRecords') {
					...
					$classname = substr(strrchr(DS . $set['class'], DS), 1, -4);
				...
				$classpath = $baseClassPath . str_replace(".", DS, $trunced) . ".php"; // removed to strip the basecp out of the equation here
				$uriclasspath = str_replace(".", DS, $trunced) . ".php"; // removed to strip the basecp out of the equation here
			}
		}
...
function executionAction (&$amfbody)
{
	...
	if (!$amfbody->isSpecialHandling() || $amfbody->isSpecialHandling(array('describeService', 'pageFetch', 'RemotingMessage')))
	{
		...
		{
			include_once(AMFPHP_BASE . 'util' .DS. 'DescribeService.php');
