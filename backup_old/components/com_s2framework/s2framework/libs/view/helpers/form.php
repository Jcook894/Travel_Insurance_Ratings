...
class FormHelper extends MyHelper {
	...
	function label($fieldName = null, $text = null, $attributes = array()) {
		...
		$labelFor = $attributes['for']!='' ? $attributes['for'] : false;
	...
	function inputs($fields = null, $attributes = null, $blacklist = null, $array = null) {
		...
		foreach ($fields as $name => $options) {
			...
			if(!$array) {
				...
				$out[$options['id']] = $this->input($name, $options);				
			}
		}
...
/*		if ($type != 'hidden') {
	...
	function text($fieldName, $attributes = array())
	{
		...
		return sprintf($this->Html->tags['text'], $fieldName, $this->_parseAttributes($attributes));
	}
	...
	function checkbox($fieldName, $options = array(), $attributes = array()) {
		...
		if(is_array($options)) 
		{
			foreach ($options as $optValue => $optTitle) 
			{
				...
				if (isset($value) && in_array($optValue, $value)) {
				...
				$tagName = $matches[0].'_'.$optValue;

				
	//			if ($label) {
	//				$checkbox = sprintf($this->Html->tags['checkboxmultiple'], $fieldName, $tagName, $parsedOptions, $optTitle);
	//				$label =  sprintf($this->Html->tags['label'], $tagName, null, $checkbox);
	//				$out[] = $label;
	//			} else {
					$checkbox = sprintf($this->Html->tags['checkboxmultiple'], $fieldName, $tagName, $parsedOptions, $optTitle);
					$label =  $this->Html->div($fieldOptionClass,$checkbox);
					$out[] = $label;				
	...
	function button($caption = null, $options = array()) {
		...
		$out = $this->output(sprintf($this->Html->tags['button'], $this->_parseAttributes($options)));
