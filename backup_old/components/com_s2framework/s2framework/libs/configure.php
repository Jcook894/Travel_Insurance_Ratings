 ...
 * @access public
 */
    function &getInstance($app = false) { 
 ...
 */
   ...
   function __listFiles($basePath,$relPath,$recursive,$type) {
        ...
        $Folder = &new Folder($basePath.$relPath);
        $contents = $Folder->read(false, array('.htaccess','index.html','index.php'));
        ...
        if (is_array($contents)) {
            ...
            foreach ($contents[1] as $item) {
                if($recursive){
                    $this->__items[$type][$relPath][strtolower($item)] = rtrim($basePath.$relPath,DS).DS.$item;                    
                } else {
                    $this->__items[$type][strtolower($item)] = rtrim($basePath.$relPath,DS).DS.$item;
                }
            }
        }
    }    
            ...
            foreach($basePaths AS $basePath){ 
                ...
                if(is_array($tree)){
                    foreach($tree[1] AS $file){
                        ...
                        if(count($path)>2)
                            $items['Theme'][array_shift($path)][array_shift($path)][strtolower(implode(DS,$path))] = $file;
                    }
                } 
            }
 ...
 * @access public
 */
    function import($type = null, $name = null, $app = 'jreviews') {
                            
        if(is_array($name)){
            foreach($name AS $n){
                App::import($type,$n,$app);        
            }
        } else {
            ...
            switch($type) {
                case 'Controller':
                    $name = $name . '_controller.php';                 
                break;
                case 'AdminController':
                    $name = $name . '_controller.php';                 
                    break;
                default: $name = $name .'.php';
            }               
...
//            prx($_this->{$app.'Paths'});
            ...
            if(isset($_this->{$app.'Paths'}[$type][$name])){
                $file = $_this->{$app.'Paths'}[$type][$name]; 
                if($_this->__load($file)) {
                    return true;        
                }
            } else {
                if($type == 'Vendor'){
                    if($_this->__load(VENDORS . $name)) {
                        return true;
                    }
                }            
            }
...
//                    App::import('Core', 'Model', S2Paths::get($app,'S2_APP'), false, Configure::corePaths('model'));
//                }
...
//                }
                if ($plugin) {
                    if (!class_exists($name . 'AppModel')) {
                        App::import($type, $plugin . '.' . $name . 'AppModel', S2Paths::get($app,'S2_APP'), false, array(), $plugin . DS . $plugin . '_app_model.php');
                    }
...
//                App::import($type, 'AppController', false);
                if ($plugin) {
                    App::import($type, $plugin . '.' . $name . 'AppController', S2Paths::get($app,'S2_APP'), false, array(), $plugin . DS . $plugin . '_app_controller.php');
